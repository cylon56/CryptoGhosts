...
contract('CryptoGhostsMarket-bid', function (accounts) {
  it("attempt to bid on an unclaimed to Ghost", async function () {
    var contract = await CryptoGhostsMarket.deployed();
    // Open up the contract for action
    await contract.allInitialOwnersAssigned();
    try {
        await contract.getGhost(0, {from: accounts[0]});
        await contract.getGhost(1, {from: accounts[1]});
        assert(false, "Should have thrown exception.");
    } catch (err) {
        // Should catch an exception
    }
    // Should fail because it is unclaimed
    await expectThrow(contract.enterBidForGhost(2, {from: accounts[0], value: 1}));
  }),
  it("attempt to bid on your own Ghost", async function () {
    var contract = await CryptoGhostsMarket.deployed();
    await expectThrow(contract.enterBidForGhost(0, {from: accounts[0], value: 1}));
  }),
  it("attempt to bid with zero value", async function () {
    var contract = await CryptoGhostsMarket.deployed();
    await expectThrow(contract.enterBidForGhost(1, {from: accounts[0], value: 0}));
  }),
  it("do a real bid", async function () {
    var contract = await CryptoGhostsMarket.deployed();
    var account0BalancePrev = await web3.eth.getBalance(accounts[0]);
    await contract.enterBidForGhost(1, {from: accounts[0], value: 1000});
  ...
  it("wrong address tries to cancel bid", async function () {
    var contract = await CryptoGhostsMarket.deployed();
    await expectThrow(contract.withdrawBidForGhost(1, {from: accounts[2]}));
  }),
  it("cancel bid", async function () {
    var contract = await CryptoGhostsMarket.deployed();
    var account0BalancePrev = await web3.eth.getBalance(accounts[0]);
    await contract.withdrawBidForGhost(1, {from: accounts[0]});
    var account0Balance = await web3.eth.getBalance(accounts[0]);
    compareBalance(account0BalancePrev, account0Balance, 1000);
    var bid = await contract.GhostBids.call(1);
  ...
  it("do another real bid", async function () {
    var contract = await CryptoGhostsMarket.deployed();
    var account0BalancePrev = await web3.eth.getBalance(accounts[0]);
    await contract.enterBidForGhost(1, {from: accounts[0], value: 2000});
    var account0Balance = await web3.eth.getBalance(accounts[0]);
    compareBalance(account0BalancePrev, account0Balance, -2000);
  }),
  it("bid underneath an existing bid", async function () {
    var contract = await CryptoGhostsMarket.deployed();
    await expectThrow(contract.enterBidForGhost(1, {from: accounts[2], value: 500}));
  }),
  it("outbid", async function () {
    var contract = await CryptoGhostsMarket.deployed();
    var account2BalancePrev = await web3.eth.getBalance(accounts[2]);
    await contract.enterBidForGhost(1, {from: accounts[2], value: 3000});
  ...
  it("wrong owner tries to accept bid", async function () {
    var contract = await CryptoGhostsMarket.deployed();
    await expectThrow(contract.acceptBidForGhost(1, 3000, {from: accounts[0]}));
  }),
  it("try to accept bid for a Ghost that has no bid", async function () {
    var contract = await CryptoGhostsMarket.deployed();
    await expectThrow(contract.acceptBidForGhost(0, 3000, {from: accounts[0]}));
  }),
  it("try to accept bid for a Ghost with too high an accept value", async function () {
    var contract = await CryptoGhostsMarket.deployed();
    await expectThrow(contract.acceptBidForGhost(1, 4000, {from: accounts[1]}));
  }),
  it("accept bid from A2", async function () {
    var contract = await CryptoGhostsMarket.deployed();

    var bid = await contract.GhostBids.call(1);
    var currentOwner = await contract.GhostIndexToAddress.call(1);
    console.log("Current owner: "+currentOwner);
    console.log("Bid: "+bid);
    await contract.acceptBidForGhost(1, 3000, {from: accounts[1]});
    console.log("Bid accepted");
    // Was A1 paid?
    var amount = await contract.pendingWithdrawals.call(accounts[1]);
    assert.equal(3000, amount);
    // Does A2 own the Ghost
    var owner = await contract.GhostIndexToAddress.call(1);
  ...
  it("offer up a Ghost for sale, then get a lower bid, accept that bid", async function () {
    var contract = await CryptoGhostsMarket.deployed();
    await contract.offerGhostForSale(0, 9000, {from: accounts[0]});
    await contract.enterBidForGhost(0, {from: accounts[2], value: 5000});
    await contract.acceptBidForGhost(0, 5000, {from: accounts[0]});
