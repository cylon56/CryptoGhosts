...
contract('CryptoGhostsMarket-fullRun', function (accounts) {
    it("should have 10000 punks available to assign", function () {
        return CryptoGhostsMarket.deployed().then(function (instance) {
        ...
        it("should be able to complete the full buy and sell cycle", function () {
            ...
            return CryptoGhostsMarket.deployed().then(function (instance) {
                ...
                return contract.transferGhost(accounts[1], 1, {from: accounts[1]});
                ...
                var promises = [];
                for (var i=0; i < 100; i++) {
                    promises.push(contract.getGhost(1001+i, {from: accounts[1]}));
                }
                ...
                Promise.all(promises).then(function() {
                    ...
                    return contract.offerGhostForSale(1001, 10000, {from: accounts[1]});
                }).then(function () {
                    return contract.punksOfferedForSale(1001);
                }).then(function (offer) {
                    console.log("Offer for sale: "+offer);
                    assert.isOk(offer[0], "Ghost was not actually for sale.");
                    assert.equal(offer[3], 10000, "Ghost sale price incorrect.");
                    assert.equal(offer[4], NULL_ACCOUNT, "Ghost should be for sale to anyone.");
                    // Get account 0 to buy a punk, but send too little ether and expect an exception
                    return contract.buyGhost(1001, {from: accounts[0], value: 1000});
                }).then(function () {
                    ...
                    return contract.buyGhost(1001, {from: accounts[0], value: 10000});
                }).then(function (address) {
                    ...
                    console.log("Offer for sale: "+offer);
                    assert.equal(offer[0], false, "Ghost was still for sale.");

                    console.log("Making sure punk 1001 can't be bought.");
                    return contract.buyGhost(1001, {from: accounts[2], value: 10000});
                }).then(function () {
                    // console.log("Bought punk.");
                    assert(false, "Was supposed to throw but didn't.");
                }).catch(function (error) {
                    ...
                    return contract.offerGhostForSaleToAddress(1001, 10000, accounts[2], {from: accounts[0]});
                   // return contract.punkIndexToAddress(1001);
                }).then(function (address) {
                    console.log("Try to get account 1 to buy punk 1001 but fail.");
                    return contract.buyGhost(1001, {from: accounts[1], value: 10000});
                }).then(function () {
                    // console.log("Bought punk.");
                    assert(false, "Was supposed to throw but didn't.");
                }).catch(function (error) {
                    ...
                    return contract.offerGhostForSale(1001, 10000, {from: accounts[2]});
                }).then(function (address) {
                    ...
                    return assert.equal(Number(subCurrBalance), Number(subPrevBalance) + 10000, "Account 1 balance incorrect after withdrawal.");
                        ...
                        // return contract.nextGhostIndexToAssign();
        });
    }),
